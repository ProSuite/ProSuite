...
namespace ProSuite.QA.Container.Test.TestSupport
{
	...
	public class MultiTableViewTest
	{
		...
		public void CanMatchRows()
		{
			...
			ESRI.ArcGIS.Geodatabase. IFeatureClass fc1 = CreateFeatureClass("CanMatchRows_fc1",
			                                       esriGeometryType.esriGeometryPolygon,
			                                       FieldUtils.CreateTextField(textField1, 1),
			                                       FieldUtils.CreateTextField(textFieldBoth, 1));
			ESRI.ArcGIS.Geodatabase.IFeatureClass fc2 = CreateFeatureClass("CanMatchRows_fc2",
			                                       esriGeometryType.esriGeometryPolygon,
			                                       FieldUtils.CreateTextField(textFieldBoth, 1),
			                                       FieldUtils.CreateTextField(textField2, 1));

			const bool caseSensitive = true;
			MultiTableView view = TableViewFactory.Create(
				new[] { ReadOnlyTableFactory.Create(fc1), ReadOnlyTableFactory.Create(fc2) },
				new[] { "G1", "G2" },
				"G1.TEXT1 = G2.TEXT2 AND G1.Text = 'x' AND G2.Text = 'y'",
				caseSensitive);

			ESRI.ArcGIS.Geodatabase.IFeature f1A = fc1.CreateFeature();
			f1A.Value[fc1.FindField(textField1)] = "A";
			f1A.Value[fc1.FindField(textFieldBoth)] = "x";
			f1A.Store();

			ESRI.ArcGIS.Geodatabase.IFeature f2A = fc2.CreateFeature();
			f2A.Value[fc2.FindField(textField2)] = "A";
			f2A.Value[fc2.FindField(textFieldBoth)] = "y";
			f2A.Store();

			Assert.IsTrue(view.MatchesConstraint(ReadOnlyUtils.Create(f1A), ReadOnlyUtils.Create(f2A)));
			Assert.AreEqual("G1.TEXT1 = 'A'; G2.TEXT2 = 'A'; G1.TEXT = 'x'; G2.TEXT = 'y'",
			                view.ToString(ReadOnlyUtils.Create(f1A), ReadOnlyUtils.Create(f2A)));

			ESRI.ArcGIS.Geodatabase.IFeature f1B = fc1.CreateFeature();
			f1B.Value[fc1.FindField(textField1)] = "b";
			f1B.Value[fc1.FindField(textFieldBoth)] = "x";
			f1B.Store();

			ESRI.ArcGIS.Geodatabase.IFeature f2B = fc2.CreateFeature();
			f2B.Value[fc2.FindField(textField2)] = "B"; // different case --> no match
			f2B.Value[fc2.FindField(textFieldBoth)] = "y";
			f2B.Store();

			Assert.IsFalse(view.MatchesConstraint(ReadOnlyUtils.Create(f1B), ReadOnlyUtils.Create(f2B)));
			Assert.AreEqual("G1.TEXT1 = 'b'; G2.TEXT2 = 'B'; G1.TEXT = 'x'; G2.TEXT = 'y'",
			                view.ToString(ReadOnlyUtils.Create(f1B), ReadOnlyUtils.Create(f2B)));
		}

		[Test]
		public void CanUseShapeAreaAlias()
		{
			ESRI.ArcGIS.Geodatabase.IFeatureClass fc1 = CreateFeatureClass("CanUseShapeAreaAlias_fc1",
			                                       esriGeometryType.esriGeometryPolygon);
			ESRI.ArcGIS.Geodatabase.IFeatureClass fc2 = CreateFeatureClass("CanUseShapeAreaAlias_fc2",
			                                       esriGeometryType.esriGeometryPolygon);
		...
		{
			ESRI.ArcGIS.Geodatabase.IFeatureClass fc1 = CreateFeatureClass("CanUseShapeLengthAlias_fc1",
			                                       esriGeometryType.esriGeometryPolygon);
			ESRI.ArcGIS.Geodatabase.IFeatureClass fc2 = CreateFeatureClass("CanUseShapeLengthAlias_fc2",
			                                       esriGeometryType.esriGeometryPolygon);
