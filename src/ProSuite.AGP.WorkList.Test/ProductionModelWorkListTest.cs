...
public class ProductionModelWorkListTest
{
	...
	{
		string filePath = TestDataPreparer.FromDirectory().GetPath("Work List.iwl");
		XmlWorkItemStateRepository stateRepository = CreateXmlStateRepository(filePath);

		// todo
		string gdbPath = TestDataPreparer.ExtractZip("ProductionModelWorkListTest.gdb.zip")
		                                 .ExtractTo(nameof(Can_instantiate_ProductionModelWorkList))
		                                 .GetPath();
	...
	public void Can_filter_ProductionModelWorkList_by_ErrorType()
	{
		...
		// Copy to this path because it's hard-coded in "Work List.iwl".
		string gdbPath = TestDataPreparer.ExtractZip("ProductionModelWorkListTest.gdb.zip")
		                                 .ExtractTo(nameof(Can_instantiate_ProductionModelWorkList))
		                                 .GetPath();
		using Geodatabase geodatabase = WorkspaceUtils.OpenFileGeodatabase(gdbPath);
		...
		Assert.Throws<ArgumentException>(() => geodatabase.ApplyEdits(() =>
		{
			...
			using var errorsFeatureClass =
				DatasetUtils.OpenDataset<FeatureClass>(gdb, "POLYGON");
	...
	{
		using var fc = DatasetUtils.OpenDataset<FeatureClass>(geodatabase, "PERIMETER");
	...
	{
		using var errorsFeatureClass =
			DatasetUtils.OpenDataset<FeatureClass>(geodatabase, "POLYGON");
