...
namespace ProSuite.Commons.AO.Geodatabase
{
	public static class GdbQueryUtils
	{
			...
			where T : IRow
		{
			...
			foreach (IRow row in GetRowsInList(
				         workspace, field, valueList,
			...
			where T : class
		{
			...
			IQueryFilter filter = new TableFilter()
			                      {
			...
			{
				foreach (IFeature feature in OpenCursor(featureClass, recycling, filter))
				{
					T result;
			...
			{
				foreach (IRow row in OpenCursor(objectClass, recycling, filter))
		                                         ...
		                                         bool recycle) where T : IRow
		{
			...
			if (table is IFeatureClass featureClass)
			{
				foreach (IFeature feature in OpenCursor(featureClass, recycle, filter))
				{
					rowCount++;
			...
			{
				foreach (IRow row in OpenCursor(table, recycle, filter))
				{
					rowCount++;
					yield return (T) row;
				}
		...
		[NotNull]
		public static IEnumerable<IFeature> OpenCursor([NotNull] IFeatureClass featureClass,
		                                               bool recycling,
		                                               [CanBeNull] IQueryFilter filter = null)
		{
			Assert.ArgumentNotNull(featureClass, nameof(featureClass));
		...
		[NotNull]
		public static IEnumerable<IRow> OpenCursor([NotNull] IObjectClass objectClass,
		                                           bool recycling,
		                                           [CanBeNull] IQueryFilter filter = null)
		{
			Assert.ArgumentNotNull(objectClass, nameof(objectClass));
		...
		[NotNull]
		public static IEnumerable<IRow> OpenCursor([NotNull] ITable table,
		                                           bool recycling,
		                                           [CanBeNull] IQueryFilter filter = null)
		{
			Assert.ArgumentNotNull(table, nameof(table));
