...
namespace ProSuite.Commons.AO.Geodatabase
{
	...
	public static class RowFormat
	{
		...
		[NotNull]
		public static string FormatGeometry([NotNull] IGeometry geometry)
		{
			Assert.ArgumentNotNull(geometry, nameof(geometry));

			string shapeType = GeometryUtils.Format(geometry.GeometryType);
			string spezGeoVal = null;

			switch (geometry.GeometryType)
			{
				case esriGeometryType.esriGeometryMultipoint:
					spezGeoVal =
						string.Format("(#points: {0:N0})",
						              ((IPointCollection) geometry).PointCount);
					break;
				case esriGeometryType.esriGeometryPolyline:
					spezGeoVal =
						string.Format("(length: {0:N0})", ((IPolyline) geometry).Length);
					break;
				case esriGeometryType.esriGeometryPolygon:
					spezGeoVal =
						string.Format("(area: {0:N0})", ((IArea) geometry).Area);
					break;
			}

			return spezGeoVal != null
				       ? string.Format("{0} {1}", shapeType, spezGeoVal)
				       : shapeType;
		}
