...
{
	internal class ExportQualitySpecificationsController :
		IExportQualitySpecificationsObserver
	{
		private readonly IExportQualitySpecificationsView _view;
		private readonly Latch _latch = new Latch();

		private const string _infix = ".qa";
		private const string _extension = "xml";
		private static readonly string _suffix = string.Format("{0}.{1}", _infix, _extension);

		#region Constructors

		/// <summary>
		/// Initializes a new instance of the <see cref="ExportQualitySpecificationsController"/> class.
		/// </summary>
		/// <param name="view">The view.</param>
		/// <param name="qualitySpecifications">The quality specifications.</param>
		public ExportQualitySpecificationsController(
			[NotNull] IExportQualitySpecificationsView view,
		...
		void IExportQualitySpecificationsObserver.ExportTargetChanged()
		{
			UpdateAppearance();
		}

		void IExportQualitySpecificationsObserver.FilePathChanged()
		{
			UpdateAppearance();
		}

		void IExportQualitySpecificationsObserver.DirectoryPathChanged()
		{
			UpdateAppearance();
		}

		void IExportQualitySpecificationsObserver.FilePathFocusLost()
		{
		...
		void IExportQualitySpecificationsObserver.DirectoryPathFocusLost()
		{
		...
		void IExportQualitySpecificationsObserver.OKClicked()
		{
		...
		void IExportQualitySpecificationsObserver.CancelClicked()
		{
		...
		void IExportQualitySpecificationsObserver.SelectAllClicked()
		{
		...
		void IExportQualitySpecificationsObserver.SelectNoneClicked()
		{
		...
		void IExportQualitySpecificationsObserver.SelectedItemsChanged()
		{
			if (_latch.IsLatched)
			{
				return;
		...
		void IExportQualitySpecificationsObserver.ExportWorkspaceConnectionsChanged()
		{
			UpdateAppearance();
		}
		                                           ...
		                                           [NotNull] Predicate<string> existsFile)
		{
			...
			while (true)
			{
				// can't use Path.GetExtension(), since we want to subtract the entire _suffix (.qa.xml)
				string fileNameNoExt =
					fileName.Substring(0, fileName.LastIndexOf(_suffix,
					                                           StringComparison.OrdinalIgnoreCase));

				string newFileNameNoExt = string.Format("{0}-{1}{2}", fileNameNoExt, i, _suffix);
		                                      ...
		                                      [NotNull] out string correctedFilePath)
		{
			...
			if (trimmedPath.EndsWith(_suffix))
			{
			...
			if (! fileNameWithoutExtension.EndsWith(_infix))
			{
				string correctedFileName = string.Format("{0}{1}", fileNameWithoutExtension,
				                                         _suffix);

				correctedFilePath = string.IsNullOrEmpty(directoryPath)
					                    ? correctedFileName
					                    : Path.Combine(directoryPath, correctedFileName);
				return false;
			}

			// the infix is correct, but the extension is not -> change it
		...
		private void UpdateAppearance(int selectionCount)
		{
			...
			_view.ExportSdeConnectionFilePathsEnabled = _view.ExportWorkspaceConnections;
