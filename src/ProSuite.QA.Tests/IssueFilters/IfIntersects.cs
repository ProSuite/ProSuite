...
namespace ProSuite.QA.Tests.IssueFilters
{
	...
	public class IfIntersects : IssueFilter
	{
		...
		[Doc(nameof(DocStrings.IfIntersects_0))]
		public IfIntersects(
			[Doc(nameof(DocStrings.IfIntersects_featureClass))]
		...
		public override bool Check(QaErrorEventArgs error)
		{
			...
			{
				if (! ((IRelationalOperator) errorGeometry).Disjoint(((IFeature) searched).Shape))
				{
					return true;
				}
			}
