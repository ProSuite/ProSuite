...
namespace ProSuite.DdxEditor.Framework.Commands
{
	...
	[Obsolete(
		"override AllowDelete on item instead (or if really needed, use non-generic class)")
	]
	public class DeleteItemCommand<T> : ItemCommandBase<T>, IDeleteItemCommand
		where T : Item
	{
		...
		/// <param name="item">The item.</param>
		/// <param name="applicationController">The application controller.</param>
		public DeleteItemCommand([NotNull] T item,
		                         [NotNull] IApplicationController applicationController)
			: base(item)
		{
			Assert.ArgumentNotNull(applicationController, nameof(applicationController));
