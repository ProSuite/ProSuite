...
public static class ChangeAlongClientUtils
{
		...
		IList<CutSubcurve> selectedSubcurves)
	{
		...
		result.UseNonDefaultReshapeSide = useNonDefaultReshapeSide;
		...
		CancellationToken cancellationToken)
	{
		...
		Polyline filterBuffer = curveFilterOptions.ShowExcludeReshapeLinesToleranceBuffer &&
		                        response.FilterBuffer != null
			                        ? (Polyline) ProtobufConversionUtils.FromShapeMsg(
				                        response.FilterBuffer, spatialReference)
			                        : null;

		ChangeAlongCurves result = PopulateReshapeAlongCurves(
	...
	private static CalculateReshapeLinesResponse CalculateReshapeCurvesRpc(
		...
		CancellationToken cancellationToken)
	{
		CalculateReshapeLinesRequest request = CreateCalculateReshapeLinesRequest(
		...
		[CanBeNull] Polyline filterBuffer = null)
	{
		...
		foreach (ReshapeLineMsg reshapeLineMsg in reshapeLineMsgs)
		{
	...
	{
		Geometry geometry = ProtobufConversionUtils.FromShapeMsg(shapeMsg);
