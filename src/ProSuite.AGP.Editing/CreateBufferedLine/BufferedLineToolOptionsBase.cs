...
{
	/// <summary>
	/// The shared, centralizable settings of the buffered-line family of tools. Concrete
	/// options classes (e.g. <see cref="CreateBufferedLineToolOptions"/> or the wall tool's
	/// options) fix the partial-options type used for persistence and provide their own
	/// display name; this keeps the settings themselves in one place while letting each tool
	/// persist its defaults independently and diverge later.
	/// </summary>
	/// <typeparam name="TPartial">The partial-options type serialized to the tool's defaults
	/// file.</typeparam>
	public abstract class BufferedLineToolOptionsBase<TPartial>
		: OptionsBase<TPartial>
		where TPartial : PartialCreateBufferedLineOptions, new()
	{
		...
		protected BufferedLineToolOptionsBase(
			[CanBeNull] TPartial centralOptions,
			[CanBeNull] TPartial localOptions)
		{
			CentralOptions = centralOptions;
			LocalOptions = localOptions ?? new TPartial();
