...
public static class SymbolUtils
{
	...
	public static double GetPointsPerUnit(this Unit unit, double referenceScale = 0)
	{
		...
		// ConversionFactor is meters/unit (for a linear unit):
		double meters = distanceMapUnits * unit.ConversionFactor;
	...
	/// </summary>
	public static T FindPrimitiveByName<T>(CIMSymbol symbol, string name, out string path)
		where T : CIMObject
	{
	...
	public static CIMSymbolReference GetSymbol(CIMRenderer renderer, INamedValues feature = null,
	                                           double scaleDenom = 0)
	{
		return GetSymbol(renderer, feature, scaleDenom, out _);
	}

	public static CIMSymbolReference GetSymbol(CIMRenderer renderer, INamedValues feature,
	                                           double scaleDenom,
	                                           out CIMPrimitiveOverride[] overrides)
	{
		if (renderer is null)
	...
	private static CIMSymbolReference GetSymbol(CIMSimpleRenderer renderer, INamedValues feature,
	                                            double scaleDenom,
	                                            out CIMPrimitiveOverride[] overrides)
	{
		// CIMSimpleRenderer:
	...
	private static CIMSymbolReference GetSymbol(CIMUniqueValueRenderer renderer,
	                                            INamedValues feature, double scaleDenom,
	                                            out CIMPrimitiveOverride[] overrides)
	{
	...
	private static CIMUniqueValueClass FindClass(CIMUniqueValueRenderer renderer,
	                                             INamedValues feature)
	{
	                                 ...
	                                 string propertyName, object value)
	{
		...
		{
			throw new Exception(
				$"Property '{propertyName}' not found on {primitive.GetType().Name}");
		}
		...
		string expression, CIMExpressionInfo arcade, INamedValues feature)
	{
		...
		if (arcade is { Expression.Length: > 0 })
		{
			...
			throw new NotImplementedException(
				"Arcade expression evaluation is not yet implemented");
		}
